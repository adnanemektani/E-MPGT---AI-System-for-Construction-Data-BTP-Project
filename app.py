...
def main():
    ...
    # ── Sidebar ──────────────────────────────────────────────
    with st.sidebar:
        st.image("https://i.ibb.co/tpPtBhDK/Whats-App-Image-2026-05-03-at-13-09-45.jpg", width=70)
        ...
        if st.button("Sync Gmail", use_container_width=True):
            with st.spinner("Scanning inbox..."):
                try:
                    ...
                    mail.select("inbox")
                    _, messages = mail.search(None, f'(SUBJECT "{project_name}")')
                    new_docs = []
                    for num in messages[0].split():
                        ...
                        for part in msg.walk():
                            if part.get_filename():
                                ...
                                new_docs.extend(extract_text_and_metadata([f], project_name, lot_tech, author=sender))
                    if new_docs:
                        ...
                        conv_id = str(uuid.uuid4())
                        st.session_state.conversations[conv_id] = {
                            "title": f"Gmail {project_name} {datetime.now().strftime('%H:%M')}",
                            "history": []
                        }
                        st.session_state.conv_indexes[conv_id] = (index, namespace)
                        st.session_state.current_conv_id = conv_id
